...
// getting the current date and comparing it with the expiry date
  ...
  let expiryDate = goals.subscribers[0].expiry
    if (currentDate > expiryDate) {

      // here we can a dispatch to update the db that the subscription has expired
      // this will be under the user account 
...
<span class="sr-only spinner">Loading...</span>
</div> : //currentDate > goals.subscribers[0].expiry ? <> please renew your subscription </> :
     ...
     <div className="JobCentercontainer rounded-md shadow-inherit cursor-pointer">
             ...
             <p className="text-[#717171] px-2"> Posted on: {jobs.createdAt.slice(0,10)}</p>
                 </div>
             <p className="px-2 font-semibold"> {jobs.jobTitle} </p>
                 <p> Desc: {jobs.jobDescription} </p>
                 <p> Email: {jobs.EMPLOYER_EMAIL} </p>
                 <p> Deadline: {jobs.DeadlineDate.slice(0,10)}</p>
         </div>
         </div>
}
        </>
